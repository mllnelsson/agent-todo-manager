...
**Prerequisites:** Python 3.12+, [uv](https://docs.astral.sh/uv/), Node.js 18+, `git`, `jq`.

### 1. Install the `atm` CLI

```sh
curl -fsSL https://raw.githubusercontent.com/mllnelsson/agent-todo-manager/main/install.sh | sh
```

The install script:
- Clones the repo to `~/.local/share/atm`
- Installs `atm` globally via `uv tool install`
- Builds the dashboard (`gui/`) so `atm admin serve` has assets to serve
- Runs DB migrations against `~/.local/share/atm/app.db`
- Adds `ATM_DATABASE_URL` and `ATM_GUI_DIST` to your shell rc (`~/.zshrc` or `~/.bashrc`)
...
```sh
export ATM_PROJECT_ID=<uuid from step 3>
